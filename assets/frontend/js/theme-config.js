...
var themeConfig = {
    ...
    initialize: function () {
        ...
        $('head').append($('<link rel="stylesheet">').attr('href', './assets/frontend/js/theme-config.css'));
        $this.build();
        $this.events();
    ...
    build: function () {
        ...
        var config = $('<div />')
            ...
            .append($('<h4 />').html('Settings')
                ...
                .append($('<hr />')
                    ...
                    .html(''), $('<ul />')
                    .addClass('options reset-settings header-settings')
                    .attr('data-type', 'header'))
                .append($('<hr />')
                    .addClass('theme-config-divider')
                    .html(''), $('<ul />')
                    .addClass('options reset-settings page-settings')
                    .attr('data-type', 'page'))
                .append($('<hr />')
                    .addClass('theme-config-divider')
                    .html(''), $('<ul />')
        ...
        var themeConfigHeader = this.container.find('ul[data-type=header]');
        var themeHeaderLink = $('<li />').append(
            $('<a />')
                .attr({'href': 'http://eazzy.me/html/luxy-static/','title': 'Header version'})
                .html('Static header').addClass('header-settings-link reset-settings-link')
        );
        themeConfigHeader.append(themeHeaderLink);

        //
        
        var themeConfigPage = this.container.find('ul[data-type=page]');
        var themePageLink = $('<li />').append(
            $('<a />')
                .attr({'href': 'http://eazzy.me/html/luxy-multipage/','title': 'Page version'})
                .html('Multipage version').addClass('page-settings-link reset-settings-link')
        );
        themeConfigPage.append(themePageLink);
